...
SELECT
	i."EmployeeNumber",
    c."MonthlyIncome",
    SUM(c."MonthlyIncome") OVER (ORDER BY i."EmployeeNumber") AS Running_Monthly_Income
FROM employees.emp_info i
JOIN employees.emp_comp c
	ON i."EmployeeNumber" = c."EmpID";
...
SELECT
	...
	COALESCE(LEAD(c."MonthlyIncome") OVER (PARTITION BY c."TotalWorkingYears" ORDER BY c."MonthlyIncome"), 0) AS FollowingMemberSalary,
	COALESCE(LAG(c."MonthlyIncome") OVER (PARTITION BY c."TotalWorkingYears" ORDER BY c."MonthlyIncome"), 0) AS LeadingMemberSalary
FROM employees.emp_info i
JOIN employees.emp_comp c
ON i."EmployeeNumber" = c."EmpID";