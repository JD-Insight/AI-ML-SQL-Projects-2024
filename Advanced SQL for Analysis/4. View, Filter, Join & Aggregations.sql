-- Let's use a virtual table using VIEW command to store our frequently used query.
...
FROM employees.emp_info
WHERE
	"JobSatisfaction" IN (1,2) AND	"MaritalStatus" = 'Single';
...
which might be one of the reason, for employees of that department to have a shorter life span with the company.*/
...
FROM employees.emp_info
GROUP BY "Attrition";
/* Interpretation: Comparing the average distance of employees who left vs who stayed back, it is 10.63 vs 8.92,
which makes it one of the important feature to analyse. */